...
import { predict, predictHour } from '../../../../services/prediction.service.js';
...
describe('Prediction Service', () => {
    ...
    it('predict should throw ValidationError when symbol empty', async () => {
        await expect(predict('')).rejects.toMatchObject({ message: 'Symbol is required' });
    });

    it('predict should throw ML_SERVICE_ERROR on axios error', async () => {
        const error = { response: { status: 502, data: { detail: 'Bad gateway' } } };
        mockAxios.post.mockRejectedValue(error);
        mockAxios.isAxiosError.mockReturnValue(true);

        await expect(predict('BTC')).rejects.toMatchObject({ code: 'ML_SERVICE_ERROR', statusCode: 502 });
    });
    ...
    it('predictHour should validate hour integer and range', async () => {
        await expect(predictHour('BTC', 0)).rejects.toMatchObject({ message: 'Hour must be between 1 and 24' });
        await expect(predictHour('BTC', 25)).rejects.toMatchObject({ message: 'Hour must be between 1 and 24' });
        await expect(predictHour('BTC', 2.5 as any)).rejects.toMatchObject({ message: 'Hour must be an integer' });
    });

    it('predictHour should throw ML_SERVICE_ERROR on axios error', async () => {
        const error = { response: { status: 500, data: { detail: 'Internal' } } };
        mockAxios.post.mockRejectedValue(error);
        mockAxios.isAxiosError.mockReturnValue(true);

        await expect(predictHour('BTC', 2)).rejects.toMatchObject({ code: 'ML_SERVICE_ERROR', statusCode: 500 });
    });
});
