...
export default class InversionService {
    ...
    async getPrecio(criptomoneda: string) {
        const res = await fetch(
            `https://api.coingecko.com/api/v3/simple/price?ids=${criptomoneda}&vs_currencies=usd`
        );
    ...
    async createInversion(
        ...
        const idPortafolio = await this.portafolioRepo.findIdByUserAndName(
            payload.userId,
            nombrePortafolio
        );
        if (!idPortafolio) {
            throw new AppError(
                ...
                404,
                'PORTAFOLIO_NOT_FOUND'
            );
        }
        const cryptoNormalizada = this.mapcriptomoneda(criptomoneda);
        const costoInicial = await this.calcularCostoInicial(
            cryptoNormalizada,
            cantidad
        );
    ...
    async getInversiones(nombrePortafolio: string, jwtToken: string) {
        ...
        const esPropietario = await this.portafolioRepo.existsByUserAndName(
            payload.userId,
            nombrePortafolio
        );
        if (!esPropietario) {
            throw new AppError(
                'No tienes acceso a este portafolio',
                403,
                'FORBIDDEN'
            );
        }
    ...
    async getInversionById(idInversion: number, jwtToken: string) {
        ...
        const esPropietario = await this.inversionRepo.existsByIdAndUser(
            idInversion,
            payload.userId
        );
        if (!esPropietario) {
            throw new AppError(
                'No tienes acceso a esta inversión',
                403,
                'FORBIDDEN'
            );
        }
